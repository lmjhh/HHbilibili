...
CGFloat SELECTOR_HEIGHT = 2.5; //%%% thickness of the selector bar
...
-(void)setupSegmentButtons {
    ...
    for (int i = 0; i<numControllers; i++) {
        ...
        button.tag = i+1; //%%% IMPORTANT: if you make your own custom buttons, you have to tag them appropriately
        button.backgroundColor = [UIColor whiteColor];//%%% buttoncolors
        [button.titleLabel setFont:[UIFont systemFontOfSize:16 weight:UIFontWeightUltraLight]];
        
        [button addTarget:self action:@selector(tapSegmentButtonAction:) forControlEvents:UIControlEventTouchUpInside];
        
        [button setTitleColor:BNavTintColor forState:UIControlStateNormal];
        [button setTitleColor:BMainColor forState:UIControlStateHighlighted];
...
-(void)setupSelector {
    ...
    selectionBar.backgroundColor = BMainColor; //%%% sbcolor
...
-(void)viewWillAppear:(BOOL)animated {
    if (!self.hasAppearedFlag) {
        ...
        for(UIView *subView in [self.navigationView subviews]){
            if([subView isKindOfClass:[UIButton class]]){
                UIButton *btn = (UIButton *)subView;
                if(btn.tag != self.currentPageIndex+1){
                    [btn setTitleColor:BNavTintColor forState:UIControlStateNormal];
                }
                else {
...
-(void)tapSegmentButtonAction:(UIButton *)button {
    ...
    if (!self.isPageScrollingFlag) {
        ...
        NSInteger tempIndex = self.currentPageIndex+1;
        ...
        if (button.tag > tempIndex) {
            ...
            for (int i = (int)tempIndex; i<=button.tag; i++) {
                [pageController setViewControllers:@[[viewControllerArray objectAtIndex:i-1]] direction:UIPageViewControllerNavigationDirectionForward animated:YES completion:^(BOOL complete){
                    
                    //%%% if the action finishes scrolling (i.e. the user doesn't stop it in the middle),
                    //then it updates the page that it's currently on
                    if (complete) {
                        [weakSelf updateCurrentPageIndex:i-1];
                    }
                }];
            }
        }
        ...
        //%%% this is the same thing but for going right -> left
            ...
            for (int i = (int)tempIndex; i >= button.tag; i--) {
                [pageController setViewControllers:@[[viewControllerArray objectAtIndex:i-1]] direction:UIPageViewControllerNavigationDirectionReverse animated:YES completion:^(BOOL complete){
                    if (complete) {
                        [weakSelf updateCurrentPageIndex:i-1];
                    }
                }];
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if(_isPageScrollingFlag) {
        ...
        contentOffset = BScreen_Width;
        BOOL isRight = YES;
        if(contentOffset > scrollView.contentOffset.x) {
            isRight = NO;
        }
        
        CGFloat alapha= ((xCoor-xFromCenter/[viewControllerArray count] + selectionBar.frame.size.width/2 + X_OFFSET)-(((BScreen_Width/[viewControllerArray count])*(self.currentPageIndex+1))-selectionBar.frame.size.width/2))/(BScreen_Width/[viewControllerArray count]);
        
        UIButton *left = [self.navigationView viewWithTag:self.currentPageIndex+1];
        UIButton *right = nil;
        if(isRight&& self.currentPageIndex < 3){
            right = [self.navigationView viewWithTag:self.currentPageIndex+2];
        }
        else if (!isRight && self.currentPageIndex > 0){
            right = [self.navigationView viewWithTag:self.currentPageIndex];
            alapha = -alapha;
        }

        [right setTitleColor:[UIColor colorWithCustom:226-81*(1-alapha) green:107+38*(1-alapha) blue:141] forState:UIControlStateNormal];
        [left setTitleColor:[UIColor colorWithCustom:226-81*alapha green:107+38*alapha blue:141] forState:UIControlStateNormal];
    }
